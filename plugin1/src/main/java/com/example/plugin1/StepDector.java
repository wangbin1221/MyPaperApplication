...
public class StepDector implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        if (mStepListeners!= null){ //回传给fragment
            mStepListeners.callbackFragment(event);
        }
        for (int i = 0; i < 3; i++) {
    ...
    private final int countStart = 9;
    private  int index = 0;
    /*
    * 检测步子，并开始计步
    * 1.传入sersor中的数据
    ...
    * */
    public void detectorNewStep(float values) {
        if (gravityOld == 0) {
            ...
            if (detectorPeak(values, gravityOld)) {  //检测为波峰
                timeOfLastPeak = timeOfThisPeak;
                timeOfNow = System.currentTimeMillis();
                long duration = timeOfNow - timeOfLastPeak;
                if (timeOfNow - timeOfLastPeak >= TimeInterval
                     ...
                     * */
                    if (++index > countStart){
                        mStepListeners.countStep(duration);
                    }
    ...
    public interface  StepCountListener{
        void countStep(long duration);
        void callbackFragment(SensorEvent event);
    }
